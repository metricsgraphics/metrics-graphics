...
svg.selectAll('circle.line_rollover_circle').style('opacity',0);svg.select('.active_datapoint').text('');if(args.rollout_callback){args.rollout_callback(d,i);}}}
...
+num(d[args.y_accessor]);}});}
if(args.rollover_callback){args.rollover_callback(d,i);}}}
this.rolloverOff=function(args){var svg=d3.select($(args.target).find('svg').get(0));return function(d,i){d3.selectAll($(args.target).find('svg .bar :eq('+i+')')).classed('active',false);svg.select('.active_datapoint').text('');if(args.rollout_callback){args.rollout_callback(d,i);}}}
this.init(args);return this;}
...
svg.select('.active_datapoint').text('');if(args.rollout_callback){args.rollout_callback(d,i);}}}
...
if(args.rollover_callback){args.rollover_callback(d,i);}}}
this.rolloverOff=function(args){var svg=d3.select($(args.target).find('svg').get(0));return function(d,i){d3.selectAll($(args.target).find('svg g.barplot .bar:eq('+i+')')).classed('active',false);svg.select('.active_datapoint').text('');if(args.rollout_callback){args.rollout_callback(d,i);}}}
this.init(args);return this;}
