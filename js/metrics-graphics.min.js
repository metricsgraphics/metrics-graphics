...
return this}
function process_line(args){var is_time_series=($.type(args.data[0][0][args.x_accessor])=='date')?true:false;if(args.missing_is_zero&&args.chart_type=='line'&&is_time_series){for(var i=0;i<args.data.length;i++){var first=args.data[i][0];var last=args.data[i][args.data[i].length-1];var processed_data=[];processed_data.push(clone(args.data[i][0]));var start_date=clone(first['date']).setDate(first['date'].getDate()+1);for(var d=new Date(start_date);d<=last['date'];d.setDate(d.getDate()+1)){var o={};var existing_o=null;$.each(args.data[i],function(i,val){if(Date.parse(val.date)==Date.parse(new Date(d))){existing_o=val;return false;}});if(!existing_o){o['date']=new Date(d);o[args.y_accessor]=0;processed_data.push(o);}
else{processed_data.push(existing_o);}}
processed_data.push(last);args.data[i]=processed_data;}}
return this;}
